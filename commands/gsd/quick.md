...
task_tags = [config.mosic.tags.gsd_managed, config.mosic.tags.quick]

# Derive 1-2 topic tags from task title/description
# Analyze DESCRIPTION for key topics (e.g., "add email notifications" → email, notifications)
# Use lowercase, hyphenated format. Skip generic terms.
# Search-then-create pattern:
derived_tags = ["{topic_1}", ...]  # 1-2 tags from task description
...
mosic_batch_add_tags_to_document("MTask", TASK_ID, task_tags)
